...
// Bullet settings
#define INVADERS_MAX_BULLETS 12
struct Bullet {
  int x, y;
  bool active;
};
...
void spacegame() {
  ...
  if (invadersPlayerAlive) {
    ...
    if (!digitalRead(BTN_RIGHT) && invadersPlayerX < 120) {
      invadersPlayerX += 2;
    }
    if (!digitalRead(BTN_SELECT)) {
      invadersShoot();
      delay(150);  // Fire rate limit
    }
  } else if (invadersExploding && (now - invadersExplosionStart > 1000)) {
    // Restart
  ...
  if (now - invadersLastEnemyMove > 100) {
    invadersMoveEnemies();
    invadersLastEnemyMove = now;
  }

  if (now - invadersLastEnemySpawn > 700 && invadersPlayerAlive) {
    invadersSpawnEnemy();
