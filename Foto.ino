...
// Submenu options
const char* colorMenu[] = {"Red", "Orange", "Blue", "Black"};
int totalColors = sizeof(colorMenu) / sizeof(colorMenu[0]);
int currentColor = 0;
bool inSubMenu = false; // Flag to indicate if we are in the submenu
  ...
  if (!inSubMenu) {
    if (autoMode) {
      ...
      if (digitalRead(BUTTON_SELECT) == LOW) {
        // Enter submenu for the selected image
        inSubMenu = true;
        currentColor = 0;  // Reset submenu selection
        displaySubMenu();
        delay(200);  // Debounce delay
      }
    ...
    // Submenu logic
    if (digitalRead(BUTTON_UP) == LOW) {
      currentColor = (currentColor - 1 + totalColors) % totalColors;  // Previous color
      displaySubMenu();
      delay(200);  // Debounce delay
    }
    if (digitalRead(BUTTON_DOWN) == LOW) {
      currentColor = (currentColor + 1) % totalColors;  // Next color
      displaySubMenu();
      delay(200);  // Debounce delay
    }
    if (digitalRead(BUTTON_SELECT) == LOW) {
      // Perform action for the selected color
      performColorAction(colorMenu[currentColor]);
      delay(200);  // Debounce delay
    }
    if (digitalRead(BUTTON_BACK) == LOW) {
      inSubMenu = false;  // Exit submenu and return to main menu
      displayManualImage(currentManualImage);
      delay(200);  // Debounce delay
    }
  }
...
void displaySubMenu() {
  LcdClear();
  gotoXY(0, 0);  // Start from top-left corner
  for (int i = 0; i < totalColors; i++) {
    if (i == currentColor) {
      LcdWrite(LCD_D, '>');  // Highlight the current selection
    } else {
      LcdWrite(LCD_D, ' ');
    }
    LcdWrite(LCD_D, colorMenu[i]);
    LcdWrite(LCD_D, '\n');
  }
}

// Perform action for the selected color
void performColorAction(const char* color) {
  LcdClear();
  gotoXY(0, 0);
  LcdWrite(LCD_D, "Selected: ");
  LcdWrite(LCD_D, color);
  if (strcmp(color, "Red") == 0) {
    digitalWrite(led, HIGH);
    delay(1000);
    digitalWrite(led, LOW);
  }
  // Add other actions for Orange, Blue, and Black here
}
